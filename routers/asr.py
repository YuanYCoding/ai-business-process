...
@router.get("/model-download-state", response_model=dict)
async def get_model_download_state():
    """Get current ASR model download progress."""
    return asr_service.get_model_download_state()
...
async def list_asr_results(
    ...
    session_only: bool = Query(default=False, description="Only return current session results")
):
    """List all ASR results. Set session_only=true to get only current session results."""
    if session_only:
        return asr_service.get_current_session_results()
