...
def download_audio_from_urls(
    ...
    """Download audio files from URLs using multi-threading."""
    ...
    def _download_one(url: str) -> AudioFile:
        ...
        try:
            ...
            with _download_lock:
                ...
                _download_progress[url].error_message = str(e)
            if progress_callback:
                progress_callback(url, "error", str(e))
            return AudioFile(filename="", path="", status=AudioStatus.ERROR, error_message=str(e))
