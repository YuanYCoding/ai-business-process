...
def generate_mermaid(structured_data: Dict) -> str:
    ...
    def _process_node(node: Dict, parent_id: str = "", depth: int = 0) -> List[str]:
        ...
        if parent_id:
            label = _sanitize(node.get("condition", ""), 30)
    ...
    # Add styling
    lines.append("")
    lines.append("    classDef question fill:#f9f,stroke:#333,stroke-width:2px")
    lines.append("    classDef decision fill:#ff9,stroke:#333,stroke-width:2px")
    lines.append("    classDef script fill:#9f9,stroke:#333,stroke-width:2px")
    lines.append("    classDef action fill:#9cf,stroke:#333,stroke-width:2px")
    lines.append("    classDef start fill:#fcc,stroke:#333,stroke-width:2px")
    lines.append("    classDef end fill:#fcc,stroke:#333,stroke-width:2px")
...
<style>
  ...
  #chart {{ width: 100%; height: 800px; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }}
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .flow-stat-item strong {{ color: #667eea; }}
      ...
      setTimeout(function() {{
        ...
        chart.setOption({{
          ...
          series: [{{ type: 'tree', data: [data], orient: 'LR', top: '2%', left: '5%', bottom: '2%', right: '15%',
            symbolSize: 8, label: {{ fontSize: 11 }}, expandAndCollapse: true }}]
        }});
      }}, 100);
    }}
  }}
